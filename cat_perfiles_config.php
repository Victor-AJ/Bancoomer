...
<script type="text/javascript">
	...
	$(function() {        
        loadHtmlAjax(true, $('#divGrid2'), "cat_perfiles_config_direcciones.php?tx_p1=<? echo $tx_p1 ?>&tx_p2=<? echo $tx_p2 ?>&tx_p3=<? echo $tx_p3 ?>&tx_p4=<? echo $tx_p4 ?>&tx_p5=<? echo $tx_p5 ?>");       
	});
    ...
    function addButtonEvents(){
        $("#btnNew").click(function(){
            if(!editing){
                var datarow = {id:"0",perfil:"",opcion:"",tx_p1:"",tx_p2:"",tx_p3:"",tx_p4:"",tx_p5:"",tx_indicador:"ACTIVO",fecha_mod:"",idusuario_mod:"",fecha_alta:"",idusuario_alta:""};
		...
		function fieldsReq(){			
			...
			var va_opcion='#'+id+'_opcion';			
			
			//alert($(va_usuario).val());	
			var error = true;
			validSelect($(va_perfil), $("#divError1")); 
			validSelect($(va_opcion), $("#divError1")); 						
			
			if ( !validSelect($(va_perfil), $("#divError1")) || !validSelect($(va_opcion), $("#divError1"))) error=false;			
			...
			if(fieldsReq()){	
			 	if(editing){
					...
					if (adding) var dispatch ="insert";
				 	...
				 	var url = "process_perfil_opciones.php?dispatch="+dispatch+"&id="+id+"&";
                	url += $("#catalogForm1").serialize();	
					//alert(url);
        ...
        $("#btnDelete").click(function(){
            if(!editing){        
                ...
                var fAceptar = function(){
					...
					var url = "process_perfil_opciones.php?dispatch=delete&id="+gr;
					//alert (url);
					executeAjax("post", false ,url, "json", func);
...
<div class="ui-widget-header align-center">CONFIGURACIÓN DEL PERFIL</div>
<div id="tabs">
    <ul>
        <li><a href="#tabs-1">Opciones de Menus</a></li>
        <li><a href="#tabs-2">Direcciones Corporativas</a></li>       
    </ul>
	<div id="tabs-1">
    <div id="divGrid1" style="width:100%" class="ui-widget ui-widget-content ui-corner-all">
    <form id="catalogForm1" method="" action="">    	
    ...
    <input id="tx_p5" name="tx_p5" type="hidden" value="<? echo $tx_p5 ?>" />
    	...
    	<div id="divError1"></div>         
            ...
            <script type="text/javascript">
				...
				(function($){				
					...
					jQuery("#list1").jqGrid({
                        caption:"Configuración del Perfil con las Opciones del Menú",
                        mtype: "GET",                       
						url:'process_perfil_opciones.php?dispatch=load',
						//alert ("Envio"+url);
                        editurl:'process_perfil_opciones.php?dispatch=delete&id',
                        datatype: "json",
                        colNames:['Edo.','Id','Perfil','Opción del Menú','Consultar','Insertar','Actualizar','Borrar','Exportar','Indicador','Fecha modifica', 'Usuario modifica','Fecha alta','Usuario alta'],
                        colModel:[   
                            {name: 'edo', index: 'edo', width: 20, align:"center", sortable: false, formoptions:{label: "Estado", rowpos:1 } },
                            {name:'id',index:'id_perfil_opcion',width:20, align:"center", editable:false,
                                editoptions:{readonly:true,size:10}, formoptions:{label: "Id", rowpos:2 }                                
                            },
                            {name:'perfil',index:'b.tx_nombre',width:200, align:"left", editable:true, 
								formoptions:{label: "Perfil", rowpos:3},
								edittype:'select',
   								editoptions:{dataUrl:'cat_carga.php?dispatch=perfil', defaultValue:'id_perfil'},	
								searchoptions:{sopt:['eq','ne','in','ni','bw','bn','ew','en','cn','nc']}
                            },   
                            {name:'opcion',index:'c.tx_nombre',width:250, align:"left", editable:true, 
								formoptions:{label: "Opción del Menú", rowpos:4},
								edittype:'select',
   								editoptions:{dataUrl:'cat_carga.php?dispatch=opcion', defaultValue:'id_opcion'},	
								searchoptions:{sopt:['eq','ne','in','ni','bw','bn','ew','en','cn','nc']}
                            },   
							{name:'tx_p1',index:'tx_p1',width:80, align:"center", editable:true,
								formoptions:{label: "Consultar", rowpos:5},
                                edittype:"select",
                                editoptions:{value:"1:SI;0:NO", size:10},
                                editrules:{required:true},                                
                                searchoptions:{sopt:['eq']}
                            },
							{name:'tx_p2',index:'tx_p2',width:70, align:"center", editable:true,
								formoptions:{label: "Insertar", rowpos:6},
                                edittype:"select",
                                editoptions:{value:"1:SI;0:NO", size:10},
                                editrules:{required:true},                                
                                searchoptions:{sopt:['eq']}
                            },      
							{name:'tx_p3',index:'tx_p3',width:80, align:"center", editable:true,
								formoptions:{label: "Actualizar", rowpos:7},
                                edittype:"select",
                                editoptions:{value:"1:SI;0:NO", size:10},
                                editrules:{required:true},                                
                                searchoptions:{sopt:['eq']}
                            },      
							{name:'tx_p4',index:'tx_p4',width:70, align:"center", editable:true,
								formoptions:{label: "Borrar", rowpos:8},
                                edittype:"select",
                                editoptions:{value:"1:SI;0:NO", size:10},
                                editrules:{required:true},                                
                                searchoptions:{sopt:['eq']}
                            },
							{name:'tx_p5',index:'tx_p5',width:70, align:"center", editable:true,
								formoptions:{label: "Exportar", rowpos:9},
                                edittype:"select",
                                editoptions:{value:"1:SI;0:NO", size:10},
                                editrules:{required:true},                                
                                searchoptions:{sopt:['eq']}
							...
							{name:'tx_indicador',index:'a.tx_indicador',width:80, align:"center", editable:true,
								formoptions:{label: "Indicador", rowpos:10},
                                edittype:"select",
                        ...
                        rowList:[50,100,150],  // opciones de filas por pagina
                        autowidth: true,    // Ancho automatico para columnas
						//width:$("#gview_list1").width(),
                        //height:360,
						height:$("#CenterPane").height()-$("#NorthPane").height()-160,
						shrinkToFit :false,
            			sortable: true,
                        ...
                        loadComplete: function(){
                            ...
                            for(var i=0;i<ids.length;i++){
								...
								// Consulta
								con = jQuery("#list1").getCell(ids[i],"tx_p1");
                                if(con == '1'){                                    
									con_r = "SI";
                                }else{
									con_r = "NO";
                                }
								jQuery("#list1").setRowData(ids[i],{tx_p1:con_r})
								
								// Insertar
								ins = jQuery("#list1").getCell(ids[i],"tx_p2");
                                if(ins == '1'){                                    
									ins_r = "SI";
                                }else{
									ins_r = "NO";
                                }
								jQuery("#list1").setRowData(ids[i],{tx_p2:ins_r})
								
								// Actualizar
								act = jQuery("#list1").getCell(ids[i],"tx_p3");
                                if(act == '1'){                                    
									act_r = "SI";
                                }else{
									act_r = "NO";
                                }
								jQuery("#list1").setRowData(ids[i],{tx_p3:act_r})

								// Borrar
								bor = jQuery("#list1").getCell(ids[i],"tx_p4");
                                if(bor == '1'){                                    
									bor_r = "SI";
                                }else{
									bor_r = "NO";
                                }
								jQuery("#list1").setRowData(ids[i],{tx_p4:bor_r})
								
								// Exportar
								expo = jQuery("#list1").getCell(ids[i],"tx_p5");
                                if(expo == '1'){                                    
									expo_r = "SI";
                                }else {
									expo_r = "NO";
                                }
								jQuery("#list1").setRowData(ids[i],{tx_p5:expo_r})								
                            }
                        },
        ...
        <div id="tabs-2">   
       	<!--  -->     	
        	<div id="divGrid2" style="width:100%" class="ui-widget ui-widget-content ui-corner-all"></div>
       	<!--  -->     	
        </div>   
	</div>