...
$FiltroAdicFac       = $_GET['FiltroAdicFac'];
...
if ($theFacturaCSI == null || $theFacturaCSI =="")
{
$sql.= " UNION ALL ";
$sql.=" SELECT TCSI.ID_FACTURA,  TCSI.ESTATUS, TCSI.FACTURA, TCSI.IDPROVEEDOR, TCSI.PROVEEDOR, TCSI.MES, TCSI.CR, TCSI.DERRAMAS, TCSI.MONTO_USD, TCSI.MONTO_MXN ,  TGPS.CR_GPS, TGPS.DOCUMENTO, TGPS.TX_REFERENCIA, TGPS.MONTO_GPS_DESTINO , TGPS.MONTO_GPS_LOCAL ,  TGPS.REFPADRE , TGPS.REGS ";
$sql.= " FROM ";
...
if ( $FiltroAdicFac <> null && $FiltroAdicFac <>"" )
	$sql.= " where FACTURA='".$FiltroAdicFac."'  "  ;
...
$result = mysqli_query($mysql, $sql);		
	...
	//<BITACORA>
	 ...
	 $myBitacora->anotaBitacora ($mysql, $actionBita , "TBL_FACTURA_DETALLE  TBL42_GPS " , "$id_login" ,   "tx_anio=$inAnio id_mes=$inMes id_estatus_factura=$idEstatusFac id_provedor=$idProveedorFac tx_factura_csi=$theFacturaCSI tx_factura_gps=$theFacturaGPS id_archivo_g=$idArchivoGPS xls=$XLS  " , ""  ,  "inf_conciliacion_facturaCR.php");
	 //<\BITACORA>
 
$vinculoXls="javascript: enviarPopUpXls('inf_conciliacion_facturaCR.php?theGPSQueryFileFac=$idArchivoGPS&anoQueryFac=$inAnio&mesQueryFac=$inMes&estatusFac=$idEstatusFac&proveedorFac=$idProveedorFac&theFacturaCSI=$theFacturaCSI&theFacturaGPS=$theFacturaGPS&FiltroAdicFac=$FiltroAdicFac&XLS=1'); ";
