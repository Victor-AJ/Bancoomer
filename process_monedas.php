...
if 	(isset($_SESSION["sess_user"])) 
	...
	$tx_descripcion	= $_GET['tx_descripcion']; 
	...
	if ($dispatch=="load") {
		...
		$sql = "   SELECT  id_moneda, id_moneda, tx_moneda, tx_descripcion, a.tx_indicador, a.fh_mod, b.tx_nombre AS usuario_mod, a.fh_alta, c.tx_nombre AS usuario_alta " ; 
		$sql.= "     FROM  tbl_moneda a, tbl_usuario b, tbl_usuario c " ; 
		$sql.= "    WHERE a.id_usuariomod = b.id_usuario " ;
		$sql.= " 	  AND a.id_usuarioalta = c.id_usuario " ; 
		...
		while($row = mysqli_fetch_array($result,MYSQLI_ASSOC)) {
			$responce->rows[$i]['id']=$row[id_moneda];
			$responce->rows[$i]['cell']=array($row[id_moneda],$row[id_moneda],$row[tx_moneda],$row[tx_descripcion],$row[tx_indicador],$row[fh_mod],$row[usuario_mod],$row[fh_alta],$row[usuario_alta]);
			$i++;
		} 	
		
		//<BITACORA>
	 	$myBitacora = new Bitacora();
		$myBitacora->anotaBitacora ($mysql, "CONSULTA" , "TBL_MONEDA" , "$id_login" ,  "", ""  ,  "process_monedas.php");
	    //<\BITACORA>
	...
	// ============================
		...
		if ($count > 0)	{	
			$data = array("error" => true, "message" => "La Moneda que desa dar de alta ya existe !</br></br> Por favor vefique ..." );					
			...
			$sql = " INSERT INTO tbl_moneda SET " ;   
			$sql.= " tx_moneda		= '$tx_moneda', ";
			...
			{		
				...
				$myBitacora = new Bitacora();
				$myBitacora->anotaBitacora ($mysql, "ALTA" , "TBL_MONEDA " , "$id_login" ,  $valBita, ""  ,  "process_monedas.php");
				//<\BITACORA>
	...
	// ============================
		...
		$sql = " SELECT * ";
		$sql.= "   FROM tbl_moneda "; 
		$sql.= "  WHERE id_moneda <> $id "; 		
		$sql.= "    AND tx_moneda = '$tx_moneda' "; 		
		...
		if ($count > 0)	{	
			$data = array("error" => true, "message" => "La Moneda $tx_usuario ya existe!</br></br>Por favor verifique ... " );				
			echo json_encode($data);
		} else {  	
			...
			//echo "aaa", $sql;      
		  		...
		  		$myBitacora = new Bitacora();
				$valores=$myBitacora->obtenvalores ($mysql, "TBL_MONEDA" , $id);
	
	
			if (mysqli_query($mysql, $sql))
			{
			
				$myBitacora->anotaBitacora ($mysql, "MODIFICACION" , "TBL_MONEDA " , "$id_login" ,  $valores, "$id"  ,  "process_monedas.php");
	...
	// ============================
		...
		$sql = " UPDATE tbl_moneda  SET TX_INDICADOR='0' ";
		$sql.= "  WHERE id_moneda = $id ";
		...
		//<BITACORA>
	 	$myBitacora = new Bitacora();
		$myBitacora->anotaBitacora ($mysql, "BAJA" , "TBL_MONEDA" , "$id_login" ,  "", "$id"  ,  "process_monedas.php");
	    //<\BITACORA>
	...
	// ============================
		...
		$sql = "   SELECT  id_moneda, id_moneda, tx_moneda, tx_descripcion, a.tx_indicador, a.fh_mod, b.tx_moneda AS usuario_mod, a.fh_alta, c.tx_moneda AS usuario_alta " ; 
		$sql.= "     FROM  tbl_moneda a, tbl_usuario b, tbl_usuario c " ; 
		$sql.= "    WHERE a.id_usuariomod = b.id_usuario " ;
		$sql.= " 	  AND a.id_usuarioalta = c.id_usuario ".$wh ;   
		$sql.= " ORDER BY $sidx $sord " ;
		...
		while($row = mysqli_fetch_array($result,MYSQLI_ASSOC)) {
	 	...
	 	$myBitacora = new Bitacora();
	 	 $whr =str_ireplace("'", " " , $wh); 
		$myBitacora->anotaBitacora ($mysql, "CONSULTA" , "TBL_MONEDA" , "$id_login" ,  $whr, ""  ,  "process_monedas.php");
	    //<\BITACORA>
