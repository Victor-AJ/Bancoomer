...
<script type="text/javascript">
    ...
    function addButtonEvents(){
        $("#btnNew").click(function(){
            if(!editing){
				var datarow = {id_subdireccion:"0",tx_direccion:"",tx_subdireccion:"",tx_indicador:"ACTIVO",fh_ultimoacceso:"",fh_mod:"",id_usuariomod:"",fh_alta:"",id_usuarioalta:""};
		...
		function fieldsReq(){			
			...
			var va_direccion='#'+id+'_tx_direccion';	
			var va_subdireccion='#'+id+'_tx_subdireccion';		
			//alert($(va_usuario).val());	
			var error = true;
			validSelect($(va_direccion), $("#divError")); 
			validText(false, $(va_subdireccion), $("#divError"), 1);
			
			if ( !validSelect($(va_direccion), $("#divError")) || !validText(false, $(va_subdireccion), $("#divError"), 1)) error=false;			
			...
			if(fieldsReq()){	
			 	if(editing){
					...
					if (adding) var dispatch ="insert";
				 	...
				 	var url = "process_subdirecciones.php?dispatch="+dispatch+"&id="+id+"&";
        ...
        $("#btnDelete").click(function(){
            if(!editing){        
                ...
                var fAceptar = function(){
					...
					var url = "process_subdirecciones.php?dispatch=delete&id="+gr;
					//alert (url);
					executeAjax("post", false ,url, "json", func);
            ...
            <script type="text/javascript">
                jQuery(document).ready(function(){
                    var mygrid = jQuery("#list1").jqGrid({
                        caption:"Catálogo de Subdirecciones (CR Descripción)",
                        mtype: "GET",                       
						url:'process_subdirecciones.php?dispatch=load',
						//alert ("Envio"+url);
                        editurl:'process_subdirecciones.php?dispatch=delete&id',
                        datatype: "json",
                        colNames:['Edo.','Id','Dirección Corporativa','SubDirección','Indicador','Fecha modifica', 'Usuario modifica','Fecha alta','Usuario alta'],
                        colModel:[   
                            {name: 'edo', index: 'edo', width:30, align:"center", sortable: false },
                            {name:'id',index:'id_subdireccion',width:30, align:"center", editable:false,
                                editoptions:{readonly:true,size:10}                                
                            },
							{name:'tx_direccion',index:'tx_direccion',width:300, align:"left", editable:true, 
								formoptions:{rowpos:1},
							 	edittype:'select',								
								editoptions:{dataUrl:'cat_carga.php?dispatch=direccion', defaultValue:'id_entidad'},								
                    			searchoptions:{sopt:['eq','ne','in','ni','bw','bn','ew','en','cn','nc']}
                			},  
                            {name:'tx_subdireccion',index:'tx_subdireccion',width:400, align:"left", editable:true, 
                        ...
                        sortname: 'tx_direccion, tx_subdireccion ', // primer columna de ordenacion
					...
					{height:600,width:800,jqModal:false,closeOnEscape:true} // view options
                ).navButtonAdd('#pager1',{
                        id:"btnPin",
                        caption:"Mostrar",
