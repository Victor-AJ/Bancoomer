    ...
    function addButtonEvents(){
		...
		$("#btnNew").click(function(){
        	...
        	if(!editing){					
				...
				loadHtmlAjax(true, $("#divAlta"), "cat_computo_m.php?dispatch="+dispatch);
        	}
		});
		...
		$("#btnEdit").click(function(){		
            ...
            if(!editing){
                ...
                if(gr != null){
                    ...
                    lastsel = gr;
					jQuery("#list1").setGridState("hidden");
					loadHtmlAjax(true, $("#divAlta"), "cat_computo_m.php?dispatch="+dispatch+"&id="+gr);
                }else {
			...
			if(fieldsReq()){	
			 	if(editing){
					...
					if (adding) var dispatch ="insert";
				 	...
				 	var url = "process_computo.php?dispatch="+dispatch+"&id="+id+"&";
        ...
        $("#btnDelete").click(function(){
            if(!editing){        
                ...
                var fAceptar = function(){
					...
					var url = "process_computo.php?dispatch=delete&id="+gr;
					//alert (url);
					executeAjax("post", false ,url, "json", func);
    ...
    <form id="catalogForm" method="" action="">
        ...
        <div id="divGrid" style="padding:1%;width:98%" class="ui-widget ui-widget-content ui-corner-all">
            ...
            <script type="text/javascript">
				(function($){
					jQuery("#list1").jqGrid({
                        caption:"Cat&aacute;logo de Computo",
                        mtype: "GET",                       
						url:'process_computo.php?dispatch=load',
                        datatype: "json",
                        colNames:['Edo.','ID','Equipo','Marca','Modelo','Precio USD','Precio MXN','Obsoleto','Indicador','Fecha modifica', 'Usuario modifica','Fecha alta','Usuario alta'],
                        colModel:[   
                            {name: 'edo', index: 'edo', width: 30, align:"center", sortable: false, formoptions:{label: "Estado", rowpos:1 } },
                            {name:'id',index:'id_computo',width:30, align:"center", editable:false, editoptions:{readonly:true,size:10}, 
								formoptions:{label: "Id", rowpos:2 }                               
                            },
                            {name:'tx_equipo',index:'tx_equipo',width:150, align:"left", editable:false, formoptions:{label: "Equipo", rowpos:3 }
							},  
                            {name:'tx_marca',index:'tx_marca',width:150, align:"left", editable:false, 
                                editoptions:{size:255,maxlength: 255},
                                editrules:{required:true},
                                formoptions:{label: "Marca", rowpos:4 },
								searchoptions:{sopt:['eq','ne','in','ni','bw','bn','ew','en','cn','nc']}
                            },							
							{name:'tx_modelo',index:'tx_modelo',width:150, align:"left", editable:false, 
                                editoptions:{size:255,maxlength: 255},
                                editrules:{required:true},
                                formoptions:{label: "Modelo", rowpos:5 },
								searchoptions:{sopt:['eq','ne','in','ni','bw','bn','ew','en','cn','nc']}
                            },														
							{name:'fl_precio_usd',index:'fl_precio_usd',width:100, align:"right", editable:false, 
                                editoptions:{size:255,maxlength: 255},
                                editrules:{required:true},
                                formoptions:{label: "Precio USD", rowpos:6 },
								searchoptions:{sopt:['eq','ne','in','ni','bw','bn','ew','en','cn','nc']}
                            },							
						    {name:'fl_precio_mxn',index:'fl_precio_mxn',width:100, align:"right", editable:false, 
                                editoptions:{size:255,maxlength: 255},
                                editrules:{required:true},
                                formoptions:{label: "Precio MXN", rowpos:7 },
								searchoptions:{sopt:['eq','ne','in','ni','bw','bn','ew','en','cn','nc']}
                            },	
							{name:'tx_obsoleto',index:'tx_obsoleto',width:80, align:"center", editable:false,
                                edittype:"select",
                                editoptions:{value:"1:SI;0:NO", size:10},
                                editrules:{required:true},
                                formoptions:{label: "Obsoleto", rowpos:8 },
							...
							{name:'tx_indicador',index:'tx_indicador',width:80, align:"center", editable:false,
                                ...
                                editrules:{required:true},
                                formoptions:{label: "Indicador", rowpos:9 },
                                searchoptions:{sopt:['eq']}
                            },                                                                          
                            ...
                            {name:'id_usuariomod',index:'usuario_mod',width:180, align:"left", editable:false,
                            ...
                            {name:'id_usuarioalta',index:'usuario_alta',width:180, editable: false,
                                editoptions:{readonly:true,size:10}
                            }
                        ],
                        ...
                        sortname: 'tx_equipo, tx_marca',  // primer columna de ordenacion
                        ...
                        loadComplete: function(){
                            ...
                            for(var i=0;i<ids.length;i++){
								...
								// Equipo Obsoleto
								obs = jQuery("#list1").getCell(ids[i],"tx_obsoleto");
                                if(obs == '1'){                                    
									obs_r = "SI";
                                }else{
									obs_r = "NO";
                                }
								jQuery("#list1").setRowData(ids[i],{tx_obsoleto:obs_r})								
                            }
                        },
