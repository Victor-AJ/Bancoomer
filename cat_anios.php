...
<script type="text/javascript">
    ...
    function addButtonEvents(){
		...
		function fieldsReq(){			
			...
			var va_anio='#'+id+'_tx_anio';			
			//alert($(va_usuario).val());	
			var error = true;
			validText(false, $(va_anio), $("#divError"), 1);
			
			if ( !validSelect($(va_anio), $("#divError"))) error=false;
			return error;
		}
			...
			if(fieldsReq()){	
			 	if(editing){
					...
					if (adding) var dispatch ="insert";
				 	...
				 	var url = "process_anios.php?dispatch="+dispatch+"&id="+id+"&";
        ...
        $("#btnDelete").click(function(){
            if(!editing){        
                ...
                var fAceptar = function(){
					...
					var url = "process_anios.php?dispatch=delete&id="+gr;
					//alert (url);
					executeAjax("post", false ,url, "json", func);
            ...
            <script type="text/javascript">
                jQuery(document).ready(function(){
                    var mygrid = jQuery("#list1").jqGrid({
                        caption:"Catálogo de A&ntilde;os",
                        mtype: "GET",                       
						url:'process_anios.php?dispatch=load',
						//alert ("Envio"+url);
                        editurl:'process_anios.php?dispatch=delete&id',
                        datatype: "json",
                        colNames:['Edo.','Id','A&ntilde;o','Indicador','Fecha modifica', 'Usuario modifica','Fecha alta','Usuario alta'],
                        colModel:[   
                            {name: 'edo', index: 'edo', width: 30, align:"center", sortable: false, editable:false },
                            {name:'id',index:'id_anio',width:30, align:"center", editable:false,
                                editoptions:{readonly:true,size:10}                                
                            },
                            {name:'tx_anio',index:'tx_anio',width:80, align:"center", editable:true,                               
                                formoptions:{label: "A&ntilde;o", rowpos:1 },
								edittype:'select',								
								editoptions:{dataUrl:'cat_carga.php?dispatch=anio', defaultValue:'tx_anio'},								
							...
							{name:'tx_indicador',index:'a.tx_indicador',width:80, align:"center", editable:true,
                                ...
                                formoptions:{label: "Indicador", rowpos:2 },
                        ...
                        sortname: 'tx_anio', // primer columna de ordenacion
