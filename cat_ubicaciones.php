    ...
    function addButtonEvents(){
		...
		$("#btnNew").click(function(){
        	...
        	if(!editing){					
				...
				loadHtmlAjax(true, $("#divAlta"), "cat_ubicaciones_m.php?dispatch="+dispatch);
        	}
		});
		...
		$("#btnEdit").click(function(){		
            ...
            if(!editing){
                ...
                if(gr != null){
                    ...
                    lastsel = gr;
					jQuery("#list1").setGridState("hidden");
					loadHtmlAjax(true, $("#divAlta"), "cat_ubicaciones_m.php?dispatch="+dispatch+"&id="+gr);
                }else {
			...
			if(fieldsReq()){	
			 	if(editing){
					...
					if (adding) var dispatch ="insert";
				 	...
				 	var url = "process_ubicaciones.php?dispatch="+dispatch+"&id="+id+"&";
        ...
        $("#btnDelete").click(function(){
            if(!editing){        
                ...
                var fAceptar = function(){
					...
					var url = "process_ubicaciones.php?dispatch=delete&id="+gr;
					//alert (url);
					executeAjax("post", false ,url, "json", func);
            ...
            <script type="text/javascript">
				(function($){
					jQuery("#list1").jqGrid({
                        ...
                        mtype: "GET",                       
						url:'process_ubicaciones.php?dispatch=load',
                        datatype: "json",
                        colNames:['Edo.','ID','Pa&iacute;s','Estado','Ubicaci&oacute;n','Indicador','Fecha modifica', 'Usuario modifica','Fecha alta','Usuario alta'],
                        colModel:[   
                            {name: 'edo', index: 'edo', width: 20, align:"center", sortable: false, formoptions:{label: "Estado", rowpos:1 } },
                            {name:'id',index:'id_ubicacion',width:20, align:"center", editable:false, editoptions:{readonly:true,size:10}, 
								formoptions:{label: "Id", rowpos:2 }                               
                            },
                            {name:'tx_pais',index:'tx_pais',width:160, align:"left", editable:false, formoptions:{label: "Pa&iacute;s", rowpos:3 }
							},  
                            {name:'tx_estado',index:'tx_estado',width:160, align:"left", editable:false, 
                                editoptions:{size:255,maxlength: 255},
                                editrules:{required:true},
                                formoptions:{label: "Estado", rowpos:4 },
								searchoptions:{sopt:['eq','ne','in','ni','bw','bn','ew','en','cn','nc']}
                            },
							{name:'tx_ubicacion',index:'tx_ubicacion',width:200, align:"left", editable:false, 
                                editoptions:{size:255,maxlength: 255},
                                editrules:{required:true},
                                formoptions:{label: "Ubicaci&oacute;n", rowpos:5 },
								searchoptions:{sopt:['eq','ne','in','ni','bw','bn','ew','en','cn','nc']}
                            },
							{name:'tx_indicador',index:'tx_indicador',width:80, align:"center", editable:false,
                                ...
                                editrules:{required:true},
                                formoptions:{label: "Indicador", rowpos:6 },
                        ...
                        sortname: 'tx_pais, tx_estado, tx_ubicacion',  // primer columna de ordenacion
					...
					{height:250,width:500,jqModal:false,closeOnEscape:true} // view options
                ).navButtonAdd('#pager1',{
                        id:"btnPin",
                        caption:"Mostrar",
