    ...
    function addButtonEvents(){
		...
		$("#btnNew").click(function(){
        	...
        	if(!editing){					
				...
				loadHtmlAjax(true, $("#divAlta"), "cat_telefonia_m.php?dispatch="+dispatch);
        	}
		});
		...
		$("#btnEdit").click(function(){		
            ...
            if(!editing){
                ...
                if(gr != null){
                    ...
                    lastsel = gr;
					jQuery("#list1").setGridState("hidden");
					loadHtmlAjax(true, $("#divAlta"), "cat_telefonia_m.php?dispatch="+dispatch+"&id="+gr);
                }else {
			...
			if(fieldsReq()){	
			 	if(editing){
					...
					if (adding) var dispatch ="insert";
				 	...
				 	var url = "process_telefonia.php?dispatch="+dispatch+"&id="+id+"&";
        ...
        $("#btnDelete").click(function(){
            if(!editing){        
                ...
                var fAceptar = function(){
					...
					var url = "process_telefonia.php?dispatch=delete&id="+gr;
					//alert (url);
					executeAjax("post", false ,url, "json", func);
    ...
    <form id="catalogForm" method="" action="">
        ...
        <div id="divGrid" style="padding:1%;width:98%" class="ui-widget ui-widget-content ui-corner-all">
            ...
            <script type="text/javascript">
				(function($){
					jQuery("#list1").jqGrid({
                        caption:"Cat&aacute;logo de Telefon&iacute;a",
                        mtype: "GET",                       
						url:'process_telefonia.php?dispatch=load',
                        datatype: "json",
                        colNames:['Edo.','ID','Tipo','Marca','Modelo','Indicador','Fecha modifica', 'Usuario modifica','Fecha alta','Usuario alta'],
                        colModel:[   
                            {name: 'edo', index: 'edo', width: 30, align:"center", sortable: false, formoptions:{label: "Estado", rowpos:1 } },
                            {name:'id',index:'id_telefonia',width:30, align:"center", editable:false, editoptions:{readonly:true,size:10}, 
								formoptions:{label: "Id", rowpos:2 }                               
                            },
                            {name:'tx_tipo',index:'tx_tipo',width:150, align:"left", editable:false, formoptions:{label: "Tipo", rowpos:3 }
							},  
                        ...
                        sortname: 'tx_tipo, tx_marca',  // primer columna de ordenacion
