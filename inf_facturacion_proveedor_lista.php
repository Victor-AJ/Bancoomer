...
if 	(isset($_SESSION['sess_user']))
{
	...
	$tx_perfil 	= $_SESSION['sess_perfil']; 
...
?>
	<script type="text/javascript">			
		...
		function btnDetalleFacturaInf(valor0) {		
	  	...
	  	function btnEditFacturaCE(valor0)
		{		
			var id0="id="+valor0;
			...
			else loadHtmlAjax(true, $("#divAltaFac"), "cat_facturas_cabecera_ce.php?"+id0+dispatch); 
		}  
...
<?php
	...
	//echo "par_proveedor",$par_proveedor;
	//echo "<br/>";
	...
	if ($origen==1) {	
		...
		if ($par_proveedor <> 0) $sql.= "	  AND b.id_proveedor  		= $par_proveedor  and b.tx_indicador='1' ";
		$sql.= "      AND a.id_proveedor		= b.id_proveedor ";
		$sql.= "      AND a.id_cuenta 			= c.id_cuenta  and c.tx_indicador='1' ";	
		if ($par_cuenta <> 0) $sql.= "  AND c.id_cuenta = $par_cuenta  and c.tx_indicador='1' ";
		...
		$sql.= "      AND a.id_proveedor		= b.id_proveedor ";
		$sql.= "      AND a.id_cuenta 			= c.id_cuenta  and c.tx_indicador='1' ";
		if ($par_cuenta <> 0) $sql.= "  AND c.id_cuenta = $par_cuenta ";
		$sql.= "      AND a.id_cuenta 			= $par_cuenta ";
		$sql.= "      AND a.id_mes 				= d.id_mes   ";
		$sql.= "      AND a.id_factura_estatus	= e.id_factura_estatus ";
		if ($par_estatus <> 0)  $sql.= "  AND e.id_factura_estatus	= $par_estatus ";
		$sql.= " ORDER BY tx_anio, tx_proveedor_corto, d.id_mes, tx_cuenta ";   
    ...
    <br/>
    <fieldset>
    <legend class="ui-state-default"><em>RESULTADO DE LA BUSQUEDA ...</em></legend>    
    <br/>
	<?php
	if ($registros==0) { 
		...
		for ($a=0; $a<18; $a++)
		{
			...
			{   
				...
				case 16 : $TheField='Exportar'; 
					echo "<td width='3%' class='ui-state-highlight align-center'>$TheField</td>";					
				break;
				case 17 : if ($tx_perfil == "CONTROL ECONOMICO" OR $tx_perfil == "ADMINISTRADOR") $TheField='Editar'; 
				 		  //if ($tx_perfil == "CONTROL ECONOMICO") $TheField='Editar'; 
						  else $TheField=""; 
					echo "<td width='3%' class='ui-state-highlight align-center'>$TheField</td>";					
				break;
			}							
			...
			while ($elemento = each($TheCatalogo[$i]))					  		
				...
				if ($tx_moneda=="USD") {
					if($fl_total_detalle_usd==NULL) {
						$TheColumnCuadre = "<a href='#' style='cursor:pointer' onclick='javascript:btnDetalleFacturaInf($id_factura)'><span class='ui-icon ui-icon-help' title='Factura sin Capturar la derrama ...'></span></a>";
						$color ="ui-amarillo align-center";
					} else if($fl_diferencia_usd > 1 OR $fl_diferencia_usd < -1) {
						$TheColumnCuadre = "<a href='#' style='cursor:pointer' onclick='javascript:btnDetalleFacturaInf($id_factura)'><span class='ui-icon ui-icon-close' title='Factura sin cuadrar con la derrama ...'></span></a>";
					  	$color ="ui-rojo align-center";
					} else {
						$TheColumnCuadre = "<a href='#' style='cursor:pointer' onclick='javascript:btnDetalleFacturaInf($id_factura)'><span class='ui-icon ui-icon-check' title='Factura cuadrada con la derrama ...'></span></a>";
				...
				for ($a=0; $a<18; $a++)
				{
					...
					{   
						...
						case 16 : $TheColumn = "<a href='#' style='cursor:pointer' onclick='javascript:btnExportarFactura($id_factura)'><span class='ui-icon ui-icon-extlink' title='Presione para Exportar los datos de la Derrama ...'></span></a>";				
								echo "<td class='ui-widget-header align-center' valign='top'>$TheColumn</td>";
						break;							
						case 17 : if ($tx_perfil == "CONTROL ECONOMICO" OR $tx_perfil == "ADMINISTRADOR") $TheColumn = "<a href='#' style='cursor:pointer' onclick='javascript:btnEditFacturaCE($id_factura)'><span class='ui-icon ui-icon-pencil' title='Presione para EDITAR ...'></span></a>";				
								//if ($tx_perfil == "CONTROL ECONOMICO") $TheColumn = "<a href='#' style='cursor:pointer' onclick='javascript:btnEditFactura($id_factura)'><span class='ui-icon ui-icon-pencil' title='Presione para EDITAR ...'></span></a>";
								else $TheColumn='';				
								echo "<td class='ui-widget-header align-center' valign='top'>$TheColumn</td>";
						break;	
					}							
				...
				for ($a=0; $a<18; $a++)
				{
					...
					{   
						...
						break;
						case 9 : $TheField="";
							echo "<td colspan='10' class='ui-state-highlight'>&nbsp;</td>";					
						break;
					}							
				}	
				echo "</tr>";	
...
<?php
	...
	$valBita.="origen=$origen ";

	//<BITACORA>
	 $myBitacora = new Bitacora();
	 $myBitacora->anotaBitacora ($mysql, "CONSULTA" , "TBL_FACTURA" , "$id_login" ,   $valBita  ,"" ,"inf_facturacion_proveedor_lista.php");
	 //<\BITACORA>
