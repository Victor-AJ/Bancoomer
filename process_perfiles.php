...
if ($dispatch=="load") {
	...
	while($row = mysqli_fetch_array($result,MYSQLI_ASSOC)) {
	 ...
	 $myBitacora->anotaBitacora ($mysql, "CONSULTA" , "TBL_PERFIL" , "$id_login" ,  "", ""  ,  "process_perfiles.php");
	 //<\BITACORA>
...
// ============================
	...
	if ($count > 0)	{	
		...
		{		
			...
			$myBitacora = new Bitacora();
			$myBitacora->anotaBitacora ($mysql, "ALTA" , "TBL_PERFIL" , "$id_login" ,  $valBita, ""  ,  "process_perfiles.php");
			//<\BITACORA>
...
// ============================
	...
	if ($count > 0)	{	
		...
		//echo "aaa", $sql;      
			$myBitacora = new Bitacora();
			$valores=$myBitacora->obtenvalores ($mysql, "TBL_PERFIL", $id);
		...
		if (mysqli_query($mysql, $sql))
		{
			...
			$myBitacora->anotaBitacora ($mysql, "MODIFICACION" , "TBL_PERFIL" , "$id_login" ,  $valores, " $id"  ,  "process_perfiles.php");
...
// ============================
	...
	$sql = " UPDATE  tbl_perfil SET TX_INDICADOR='0' ";
	$sql.= "  WHERE id_perfil = $id ";
	...
	if (mysqli_query($mysql, $sql)) 
	{
	 ...
	 //<BITACORA>
	 $myBitacora = new Bitacora();
	 $myBitacora->anotaBitacora ($mysql, "BAJA" , "TBL_PERFIL" , "$id_login" ,  "", "$id"  ,  "process_usuarios.php");
	 //<\BITACORA>
...
// ============================
	...
	while($row = mysqli_fetch_array($result,MYSQLI_ASSOC)) {
	 ...
	 $myBitacora = new Bitacora();
	  $whr =str_ireplace("'", " " , $wh); 
	 $myBitacora->anotaBitacora ($mysql, "CONSULTA" , "TBL_PERFIL" , "$id_login" ,  $whr, ""  ,  "process_perfiles.php");
	 //<\BITACORA>
