...
// ============================
	if ($dispatch=="load") 
	{
		...
		$sql.= "   FROM tbl_anio a, tbl_usuario b, tbl_usuario c ";
		...
		$sql = "   SELECT  a.id_anio, a.id_anio, a.tx_anio, a.tx_indicador, a.fh_mod, b.tx_nombre AS usuario_mod, a.fh_alta, c.tx_nombre AS usuario_alta " ; 
		$sql.= "     FROM  tbl_anio a, tbl_usuario b, tbl_usuario c " ; 
		$sql.= "    WHERE a.id_usuariomod = b.id_usuario " ;
		$sql.= " 	  AND a.id_usuarioalta = c.id_usuario " ; 
		...
		{
			$responce->rows[$i]['id']=$row[id_anio];
			$responce->rows[$i]['cell']=array($row[id_anio],$row[id_anio],$row[tx_anio],$row[tx_indicador],$row[fh_mod],$row[usuario_mod],$row[fh_alta],$row[usuario_alta]);
			$i++;
		} 	
		
			//<BITACORA>
			$myBitacora = new Bitacora();
			$myBitacora->anotaBitacora ($mysql, "CONSULTA" , "TBL_ANIO" , "$id_login" ,  "", ""  ,  "process_anios.php");
			//<\BITACORA>
	...
	// ============================
		...
		{	
			...
			if ($count > 0)	
			{	
				$data = array("error" => true, "message" => "El A&ntilde;o que desa dar de alta ya existe !</br></br> Por favor vefique ..." );					
			...
			{  						
				...
				$myBitacora = new Bitacora();
				$myBitacora->anotaBitacora ($mysql, "ALTA" , "TBL_ANIO" , "$id_login" ,  $valBita, ""  ,  "process_anios.php");
				//<\BITACORA>
			...
			// ============================
				...
				{	
					...
					if ($count > 0)	
					{	
						$data = array("error" => true, "message" => "El A&ntilde;o $tx_anio ya existe!</br></br>Por favor verifique ... " );				
						echo json_encode($data);
					} 
					else 
					{  	
						...
						//echo "aaa", $sql;      
							$myBitacora = new Bitacora();
							$valores=$myBitacora->obtenvalores ($mysql, "TBL_ANIO", $id);
				
						if (mysqli_query($mysql, $sql))
						{
				
							$myBitacora->anotaBitacora ($mysql, "MODIFICACION" , "TBL_ANIO" , "$id_login" ,  $valores, "$id"  ,  "process_anios.php");
							//<\BITACORA>
				...
				// ============================
					...
					{		
						$sql = " update  tbl_anio  set tx_indicador='0' ";
						$sql.= "  WHERE id_anio = $id ";
						...
						if (mysqli_query($mysql, $sql)) 
						{
							//<BITACORA>
							$myBitacora = new Bitacora();
							$myBitacora->anotaBitacora ($mysql, "BAJA" , "TBL_ANIO" , "$id_login" ,  "", "$id"  ,  "process_anios.php");
							//<\BITACORA>
					...
					{	
						...
						$myBitacora = new Bitacora();
						$whr =str_ireplace("'", " " , $wh); 
						$myBitacora->anotaBitacora ($mysql, "CONSULTA" , "TBL_ANIO" , "$id_login" ,  $whr, ""  ,  "process_anios.php");
						//<\BITACORA>
