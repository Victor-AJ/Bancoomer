    ...
    function addButtonEvents(){
		...
		$("#btnNew").click(function(){
        	...
        	if(!editing){					
				...
				loadHtmlAjax(true, $("#divAlta"), "cat_telefonia_planes_m.php?dispatch="+dispatch);
        	}
		});
		...
		$("#btnEdit").click(function(){		
            ...
            if(!editing){
                ...
                if(gr != null){
                    ...
                    lastsel = gr;
					jQuery("#list1").setGridState("hidden");
					loadHtmlAjax(true, $("#divAlta"), "cat_telefonia_planes_m.php?dispatch="+dispatch+"&id="+gr);
                }else {
			...
			if(fieldsReq()){	
			 	if(editing){
					...
					if (adding) var dispatch ="insert";
				 	...
				 	var url = "process_telefonia_planes.php?dispatch="+dispatch+"&id="+id+"&";
        ...
        $("#btnDelete").click(function(){
            if(!editing){        
                ...
                var fAceptar = function(){
					...
					var url = "process_telefonia_planes.php?dispatch=delete&id="+gr;
					//alert (url);
					executeAjax("post", false ,url, "json", func);					
                }
    ...
    <form id="catalogForm" method="" action="">
        ...
        <div id="divGrid" style="padding:1%;width:98%" class="ui-widget ui-widget-content ui-corner-all">
            ...
            <script type="text/javascript">
				(function($){
					jQuery("#list1").jqGrid({
                        caption:"Cat&aacute;logo de Planes de  Telefon&iacute;a",
                        mtype: "GET",                       
						url:'process_telefonia_planes.php?dispatch=load',
                        datatype: "json",
                        colNames:['Edo.','ID','Proveedor','Plan','Precio','Descripci&oacute;n','Indicador','Fecha modifica', 'Usuario modifica','Fecha alta','Usuario alta'],
                        colModel:[   
                            {name: 'edo', index: 'edo', width: 30, align:"center", sortable: false, formoptions:{label: "Estado", rowpos:1 } },
                            {name:'id',index:'id_telefonia_plan',width:35, align:"center", editable:false, editoptions:{readonly:true,size:10}, 
								formoptions:{label: "Id", rowpos:2 }                               
                            },
                            {name:'tx_proveedor',index:'tx_proveedor',width:150, align:"left", editable:false, formoptions:{label: "Tipo", rowpos:3 }
							},  
                            {name:'tx_plan',index:'tx_plan',width:200, align:"left", editable:false, 
                                editoptions:{size:255,maxlength: 255},
                                editrules:{required:true},
                                formoptions:{label: "Plan", rowpos:4 },
								searchoptions:{sopt:['eq','ne','in','ni','bw','bn','ew','en','cn','nc']}
                            },							
							{name:'tx_precio_mxn',index:'tx_precio_mxn',width:100, align:"right", editable:false, 
                                editoptions:{size:255,maxlength: 255},
                                editrules:{required:true},
                                formoptions:{label: "Precio", rowpos:5 },
								searchoptions:{sopt:['eq','ne','in','ni','bw','bn','ew','en','cn','nc']}
                            },
							{name:'tx_descripcion',index:'tx_descripcion',width:350, align:"left", editable:false, 
                                editoptions:{size:255,maxlength: 255},
                                editrules:{required:true},
                                formoptions:{label: "Descripci&oacute;n", rowpos:5 },
								searchoptions:{sopt:['eq','ne','in','ni','bw','bn','ew','en','cn','nc']}
                            },							
                        ...
                        sortname: 'tx_proveedor, tx_plan',  // primer columna de ordenacion
