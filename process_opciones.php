...
if 	(isset($_SESSION["sess_user"])) 
	...
	if ($dispatch=="load") {
		...
		$sql.= "   FROM tbl_opcion a, tbl_usuario b, tbl_usuario c ";
		...
		$sql = "   SELECT  a.id_opcion, a.id_opcion, a.tx_nombre, a.tx_indicador, a.fh_mod, b.tx_nombre AS usuario_mod, a.fh_alta, c.tx_nombre AS usuario_alta " ; 
		$sql.= "     FROM  tbl_opcion a, tbl_usuario b, tbl_usuario c " ; 
		$sql.= "    WHERE a.id_usuariomod = b.id_usuario " ;
		$sql.= " 	  AND a.id_usuarioalta = c.id_usuario " ; 
		...
		while($row = mysqli_fetch_array($result,MYSQLI_ASSOC)) {
			$responce->rows[$i]['id']=$row[id_opcion];
			$responce->rows[$i]['cell']=array($row[id_opcion],$row[id_opcion],$row[tx_nombre],$row[tx_indicador],$row[fh_mod],$row[usuario_mod],$row[fh_alta],$row[usuario_alta]);
			$i++;
		} 	
		
		//<BITACORA>
	 	$myBitacora = new Bitacora();
		$myBitacora->anotaBitacora ($mysql, "CONSULTA" , "TBL_OPCIONES" , "$id_login" ,  "", ""  ,  "process_opciones.php");
	    //<\BITACORA>
	...
	// ============================
		...
		if ($count > 0)	{	
			$data = array("error" => true, "message" => "La Opción que desa dar de alta ya existe !</br></br> Por favor vefique ..." );					
			...
			$sql = " INSERT INTO tbl_opcion SET " ;   
			...
			{		
				...
				$myBitacora = new Bitacora();
				$myBitacora->anotaBitacora ($mysql, "ALTA" , "TBL_OPCION" , "$id_login" ,  $valBita, ""  ,  "process_opciones.php");
				//<\BITACORA>
	...
	// ============================
		...
		if ($count > 0)	{	
			$data = array("error" => true, "message" => "La Opción $tx_usuario ya existe!</br></br>Por favor verifique ... " );				
			echo json_encode($data);
		} else {  	
			...
			//echo "aaa", $sql;      
				...
				$myBitacora = new Bitacora();
				$valores=$myBitacora->obtenvalores ($mysql, "TBL_OPCION" , $id);
	
			  
			if (mysqli_query($mysql, $sql))
			{
			
				$myBitacora->anotaBitacora ($mysql, "MODIFICACION" , "TBL_OPCION" , "$id_login" ,  $valores, "$id"  ,  "process_opciones.php");
	...
	// ============================
		...
		$sql = " UPDATE  tbl_opcion SET TX_INDICADOR ='0' ";
		$sql.= "  WHERE id_opcion = $id ";
		...
		//<BITACORA>
	 	$myBitacora = new Bitacora();
		$myBitacora->anotaBitacora ($mysql, "BAJA" , "TBL_OPCIONES" , "$id_login" ,  "", "$id"  ,  "process_opciones.php");
	    //<\BITACORA>
	...
	// ============================
		...
		//<BITACORA>
	 	$myBitacora = new Bitacora();
	 	 $whr =str_ireplace("'", " " , $wh); 
		$myBitacora->anotaBitacora ($mysql, "CONSULTA" , "TBL_OPCIONES" , "$id_login" ,  $whr, ""  ,  "process_opciones.php");
	    //<\BITACORA>
