...
?>
	...
	<script type="text/javascript">			
		...
		function btnInformesDirFacDetMes(v1,v2)
		{
			var p1= parseInt(v1); 			
			var p2= parseInt(v2); 			
			...
			var id4="&par_factura="+p2;
			var id5 = "&par_cuenta="+$("#sel_cta_contable").val();
			
			//alert ("Entre "+id0+id1+id2+id3+id4);	
			$("#divDetalleFacDir").html("");			
			loadHtmlAjax(true, $("#divDetalleFacDir"), "inf_facturas_lista_detalle_direccion_fac.php?"+id0+id1+id2+id3+id4+id5); 
		}	 
...
<?
	...
	//SUM(b.fl_precio_usd) + SUM(b.fl_precio_mxn / a.fl_tipo_cambio) AS fl_precio_usd, 
	//SUM(b.fl_precio_usd * a.fl_tipo_cambio) + SUM(b.fl_precio_mxn ) AS fl_precio_usd, 
			
	if ($par_moneda=="USD") $sql = "   SELECT a.id_factura, tx_anio, tx_proveedor_corto, tx_cuenta, tx_factura, fh_factura, fh_inicio, fh_final, 	
	SUM(IF(a.id_moneda=1,IF(a.fl_tipo_cambio=0,(b.fl_precio_mxn/$in_tc),(b.fl_precio_mxn/a.fl_tipo_cambio)),(b.fl_precio_usd))) AS fl_precio_usd,	
	fl_tipo_cambio, tx_estatus, tx_mes ";	
	else $sql = "   SELECT a.id_factura, tx_anio, tx_proveedor_corto, tx_cuenta, tx_factura, fh_factura, fh_inicio, fh_final, 
	SUM(IF(a.id_moneda=2,IF(a.fl_tipo_cambio=0,(b.fl_precio_usd*$in_tc),(b.fl_precio_usd*a.fl_tipo_cambio)),(b.fl_precio_mxn))) AS fl_precio_usd,
	fl_tipo_cambio, tx_estatus, i.id_mes, tx_mes ";
	$sql.= "     FROM tbl_factura a, tbl_factura_detalle b, tbl_centro_costos c, tbl_direccion d,  tbl_proveedor f, tbl_factura_estatus g, tbl_cuenta h, tbl_mes i , tbl_producto p ";
	...
	if ($par_proveedor==3) $sql.= "     AND tx_proveedor_corto NOT IN ('$tx_proveedor1','$tx_proveedor2')  ";  
	else $sql.= "      AND tx_proveedor_corto	= '$tx_proveedor' "; 
	...
	$sql.= " ORDER BY tx_proveedor_corto, a.id_mes ";
				...
				for ($a=0; $a<11; $a++)
				{
					...
					{   
						...
						case 5: $TheColumn=number_format($fl_precio_usd,0); 
								...
								else $TheColumn = "<a href='#' style='cursor:pointer' onclick='javascript:btnInformesDirFacDetMes($par_direccion, $id_factura)' title='Presione para ver el detalle de $tx_proveedor_corto - Factura $tx_factura ... '>$TheColumn</a>";
...
<?
	...
	$valBita.="par_cuenta=$par_cuenta ";
	 ...
	 $myBitacora->anotaBitacora ($mysql, "CONSULTA" , "TBL_FACTURA TBL_FACTURA_DETALLE" , "$id_login" , $valBita   ,"" ,"inf_facturacion_dir_sub_mes_fac.php");
	 //<\BITACORA>
