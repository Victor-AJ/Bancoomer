    ...
    function addButtonEvents(){
    	...
    	$("#btnNew").click(function(){
        	...
        	if(!editing){					
				...
				loadHtmlAjax(true, $("#divAlta"), "cat_centro_costos_m.php?dispatch="+dispatch);
        	}
		});
        ...
        $("#btnEdit").click(function(){
            ...
            if(!editing){
                ...
                if(gr != null){
                    ...
                    lastsel = gr;
					jQuery("#list1").setGridState("hidden");
					loadHtmlAjax(true, $("#divAlta"), "cat_centro_costos_m.php?dispatch="+dispatch+"&id="+gr);
                }else {
		...
		function fieldsReq(){			
			...
			var va_subdireccion='#'+id+'_tx_subdireccion';	
			var va_departamento='#'+id+'_tx_departamento';		
			//alert($(va_subdireccion).val());	
			var error = true;
			validSelect($(va_subdireccion), $("#divError")); 
			validText(false, $(va_departamento), $("#divError"), 1);
			
			if ( !validSelect($(va_subdireccion), $("#divError")) || !validText(false, $(va_departamento), $("#divError"), 1)) error=false;			
			...
			if(fieldsReq()){	
			 	if(editing){
					...
					if (adding) var dispatch ="insert";
				 	...
				 	var url = "process_centro_costos.php?dispatch="+dispatch+"&id="+id+"&";
        ...
        $("#btnDelete").click(function(){
            if(!editing){        
                ...
                var fAceptar = function(){
					...
					var url = "process_centro_costos.php?dispatch=delete&id="+gr;
					//alert (url);
					executeAjax("post", false ,url, "json", func);
            ...
            <script type="text/javascript">
                jQuery(document).ready(function(){
                    var mygrid = jQuery("#list1").jqGrid({
                        caption:"Catálogo de Centro de Costos",
                        mtype: "GET",                       
						url:'process_centro_costos.php?dispatch=load',
						//alert ("Envio"+url);
                        editurl:'process_centro_costos.php?dispatch=delete&id',
                        datatype: "json",
                        colNames:['Edo.','Id','CR','Direcci&oacute;n','SubDirecci&oacute;n','Departamento','Estado','Indicador','Fecha modifica', 'Usuario modifica','Fecha alta','Usuario alta'],
                        colModel:[   
                            {name: 'edo', index: 'edo', width:30, align:"center", sortable: false, formoptions:{label: "Estado", rowpos:1 } },
                            {name:'id',index:'id_centro_costos',width:30, align:"center", editable:false, editoptions:{readonly:true,size:10}, 
								formoptions:{label: "Id", rowpos:2 }                                 																
                            },
							{name:'tx_centro_costos',index:'tx_centro_costos',width:60, align:"center", editable:true, 
								editoptions:{size:255,maxlength: 255},
                                editrules:{required:true},
                                formoptions:{rowpos:3 },
								searchoptions:{sopt:['eq','ne','in','ni','bw','bn','ew','en','cn','nc']}
                			},  
							{name:'tx_direccion',index:'tx_direccion',width:300, align:"left", editable:true, 
								editoptions:{size:255,maxlength: 255},
                                editrules:{required:true},
                                formoptions:{rowpos:4 },
								searchoptions:{sopt:['eq','ne','in','ni','bw','bn','ew','en','cn','nc']}
                			}, 
							{name:'tx_subdireccion',index:'tx_subdireccion',width:300, align:"left", editable:true, 
								editoptions:{size:255,maxlength: 255},
                                editrules:{required:true},
                                formoptions:{rowpos:5 },								
								searchoptions:{sopt:['eq','ne','in','ni','bw','bn','ew','en','cn','nc']}
							},	
                            {name:'tx_departamento',index:'tx_departamento',width:300, align:"left", editable:true, 
                                editoptions:{size:255,maxlength: 255},
                                editrules:{required:true},
                                formoptions:{rowpos:6 },
								searchoptions:{sopt:['eq','ne','in','ni','bw','bn','ew','en','cn','nc']}
                            },   
							{name:'tx_cr_estado',index:'tx_cr_estado',width:100, align:"left", editable:true, 
                                editoptions:{size:255,maxlength: 255},
                                editrules:{required:true},
                        ...
                        sortname: 'tx_centro_costos, tx_direccion', // primer columna de ordenacion
