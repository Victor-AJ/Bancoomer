...
if 	(isset($_SESSION["sess_user"])) 
	...
	if ($dispatch=="load") {
		...
		$sql.= "   FROM tbl_pais a, tbl_usuario b, tbl_usuario c ";
		...
		$sql = "   SELECT  a.id_pais, a.id_pais, a.tx_pais, a.tx_indicador, a.fh_mod, b.tx_nombre AS usuario_mod, a.fh_alta, c.tx_nombre AS usuario_alta " ; 
		$sql.= "     FROM  tbl_pais a, tbl_usuario b, tbl_usuario c " ; 
		$sql.= "    WHERE a.id_usuariomod = b.id_usuario " ;
		$sql.= " 	  AND a.id_usuarioalta = c.id_usuario " ; 
		...
		while($row = mysqli_fetch_array($result,MYSQLI_ASSOC)) {
			$responce->rows[$i]['id']=$row[id_pais];
			$responce->rows[$i]['cell']=array($row[id_pais],$row[id_pais],$row[tx_pais],$row[tx_indicador],$row[fh_mod],$row[usuario_mod],$row[fh_alta],$row[usuario_alta]);
			$i++;
		} 	
		
		//<BITACORA>
	 $myBitacora = new Bitacora();
	 $myBitacora->anotaBitacora ($mysql, "CONSULTA" , "TBL_PAIS" , "$id_login" ,  "", ""  ,  "process_paises.php");
	 //<\BITACORA>
	...
	// ============================
		...
		if ($count > 0)	{	
			$data = array("error" => true, "message" => "El Pa&iacute;s que desa dar de alta ya existe !</br></br> Por favor vefique ..." );					
			...
			{		
				...
				$myBitacora = new Bitacora();
				$myBitacora->anotaBitacora ($mysql, "ALTA" , "TBL_PAIS" , "$id_login" ,  $valBita, ""  ,  "process_paises.php");
				//<\BITACORA>
	...
	// ============================
		...
		if ($count > 0)	{	
			$data = array("error" => true, "message" => "El Pa&iacute;s $tx_pais ya existe!</br></br>Por favor verifique ... " );				
			echo json_encode($data);
		} else {  	
			...
			//echo "aaa", $sql;      
				$myBitacora = new Bitacora();
				$valores=$myBitacora->obtenvalores ($mysql, "TBL_PAIS" , $id);
	
			  
			if (mysqli_query($mysql, $sql))
			{
			
				$myBitacora->anotaBitacora ($mysql, "MODIFICACION" , "TBL_PAIS" , "$id_login" ,  $valores, "$id"  ,  "process_paises.php");
	...
	// ============================
		...
		$sql = " SELECT * ";
		$sql.= "   FROM tbl_estado a, tbl_pais b "; 
		$sql.= "  WHERE a.id_pais = $id "; 	
		...
		if ($count > 0)	{			
			...
			$data = array("error" => true, "message" => "El Pa&iacute;s $tx_pais esta relacionado al cat&aacute;logo de Estados, no es posible eliminarlo ... " );				
			echo json_encode($data);			
		} else {  
		
			$sql = " UPDATE  tbl_pais SET TX_INDICADOR='0' ";
			$sql.= "  WHERE id_pais = $id ";
			...
			if (mysqli_query($mysql, $sql)) 
			{
				...
				//<BITACORA>
	 			$myBitacora = new Bitacora();
				$myBitacora->anotaBitacora ($mysql, "BAJA" , "TBL_PAIS" , "$id_login" ,  "", "$id"  ,  "process_paises.php");
				//<\BITACORA>
	...
	// ============================
	 ...
	 $myBitacora = new Bitacora();
	 $whr =str_ireplace("'", " " , $wh); 
	 $myBitacora->anotaBitacora ($mysql, "CONSULTA" , "TBL_PAIS" , "$id_login" , $whr , ""  ,  "process_paises.php");
	 //<\BITACORA>
