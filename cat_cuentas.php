    ...
    function addButtonEvents(){
		...
		$("#btnNew").click(function(){
        	...
        	if(!editing){					
				...
				loadHtmlAjax(true, $("#divAlta"), "cat_cuentas_m.php?dispatch="+dispatch);
        	}
		});
		...
		$("#btnEdit").click(function(){		
            ...
            if(!editing){
                ...
                if(gr != null){
                    ...
                    lastsel = gr;
					jQuery("#list1").setGridState("hidden");
					loadHtmlAjax(true, $("#divAlta"), "cat_cuentas_m.php?dispatch="+dispatch+"&id="+gr);
                }else {
			...
			if(fieldsReq()){	
			 	if(editing){
					...
					if (adding) var dispatch ="insert";
				 	...
				 	var url = "process_cuentas.php?dispatch="+dispatch+"&id="+id+"&";
        ...
        $("#btnDelete").click(function(){
            if(!editing){        
                ...
                var fAceptar = function(){
					...
					var url = "process_cuentas.php?dispatch=delete&id="+gr;
					//alert (url);
					executeAjax("post", false ,url, "json", func);
    ...
    <form id="catalogForm" method="" action="">
        ...
        <div id="divGrid" style="padding:1%;width:98%" class="ui-widget ui-widget-content ui-corner-all">
            ...
            <script type="text/javascript">
				(function($){
					jQuery("#list1").jqGrid({
                        caption:"Cat&aacute;logo de Cuentas Proveedores",
                        mtype: "GET",                       
						url:'process_cuentas.php?dispatch=load',
                        datatype: "json",
                        colNames:['Edo.','ID','Proveedor','Cuenta','Descripci&oacute;n','Indicador','Fecha modifica', 'Usuario modifica','Fecha alta','Usuario alta'],
                        colModel:[   
                            {name: 'edo', index: 'edo', width: 30, align:"center", sortable: false, formoptions:{label: "Estado", rowpos:1 } },
                            {name:'id',index:'id_cuenta',width:30, align:"center", editable:false, editoptions:{readonly:true,size:10}, 
								formoptions:{label: "Id", rowpos:2 }                               
                            },
                            {name:'tx_proveedor',index:'tx_proveedor',width:250, align:"left", editable:false, formoptions:{label: "Proveedor", rowpos:3 }
							},  
                            {name:'tx_cuenta',index:'tx_cuenta',width:200, align:"left", editable:false, 
                                editoptions:{size:255,maxlength: 255},
                                editrules:{required:true},
                                formoptions:{label: "Cuenta", rowpos:4 },
                        ...
                        sortname: 'tx_proveedor, tx_cuenta',  // primer columna de ordenacion
