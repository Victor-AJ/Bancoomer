...
<script type="text/javascript">
    ...
    function addButtonEvents(){
        $("#btnNew").click(function(){
            if(!editing){
                var datarow = {id:"0",tx_pais:"",tx_estado:"",tx_indicador:"ACTIVO",fecha_mod:"",idusuario_mod:"",fecha_alta:"",idusuario_alta:""};
			...
			if(fieldsReq()){	
			 	if(editing){
					...
					if (adding) var dispatch ="insert";
				 	...
				 	var url = "process_estados.php?dispatch="+dispatch+"&id="+id+"&";
        ...
        $("#btnDelete").click(function(){
            if(!editing){        
                ...
                var fAceptar = function(){
					...
					var url = "process_estados.php?dispatch=delete&id="+gr;
					//alert (url);
					executeAjax("post", false ,url, "json", func);
            ...
            <script type="text/javascript">
				(function($){
					jQuery("#list1").jqGrid({
                        ...
                        mtype: "GET",                       
						url:'process_estados.php?dispatch=load',
                        datatype: "json",
                        colNames:['Edo.','ID','Pa&iacute;s','Estado','Indicador','Fecha modifica', 'Usuario modifica','Fecha alta','Usuario alta'],
                        colModel:[   
                            {name: 'edo', index: 'edo', width: 20, align:"center", sortable: false, formoptions:{label: "Estado", rowpos:1 } },
                            {name:'id',index:'id_estado',width:20, align:"center", editable:false, editoptions:{readonly:true,size:10}, 
								formoptions:{label: "Id", rowpos:2 }                               
                            },
                            {name:'tx_pais',index:'tx_pais',width:160, align:"left", editable:true, 
                                formoptions:{label: "Pa&iacute;s", rowpos:3 },
								edittype:'select',								
								editoptions:{dataUrl:'cat_carga.php?dispatch=pais', defaultValue:'id_pais'},	
								searchoptions:{sopt:['eq','ne','in','ni','bw','bn','ew','en','cn','nc']}
                            },   
                            {name:'tx_estado',index:'tx_estado',width:160, align:"left", editable:true, 
                        ...
                        sortname: 'tx_pais, tx_estado', // primer columna de ordenacion
