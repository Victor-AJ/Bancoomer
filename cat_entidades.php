...
<script type="text/javascript">
    ...
    function addButtonEvents(){
		...
		function fieldsReq(){			
			...
			var va_perfil='#'+id+'_tx_nombre';			
			//alert($(va_usuario).val());	
			var error = true;
			validText(false, $(va_perfil), $("#divError"), 1);
			
			if ( !validSelect($(va_perfil), $("#divError"))) error=false;
			return error;
		}
			...
			if(fieldsReq()){	
			 	if(editing){
					...
					if (adding) var dispatch ="insert";
				 	...
				 	var url = "process_entidades.php?dispatch="+dispatch+"&id="+id+"&";
        ...
        $("#btnDelete").click(function(){
            if(!editing){        
                ...
                var fAceptar = function(){
					...
					var url = "process_entidades.php?dispatch=delete&id="+gr;
					//alert (url);
					executeAjax("post", false ,url, "json", func);
            ...
            <script type="text/javascript">
                jQuery(document).ready(function(){
                    var mygrid = jQuery("#list1").jqGrid({
                        caption:"Cat&aacute;logo de Entidades",
                        mtype: "GET",                       
						url:'process_entidades.php?dispatch=load',
						//alert ("Envio"+url);
                        editurl:'process_entidades.php?dispatch=delete&id',
                        datatype: "json",
                        colNames:['Edo.','Id','Entidad','Indicador','Fecha modifica', 'Usuario modifica','Fecha alta','Usuario alta'],
                        colModel:[   
