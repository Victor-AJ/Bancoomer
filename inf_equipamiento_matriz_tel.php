...
<?
	...
	# ==============================================================	
	$sql1 = "   SELECT tx_tipo ";
	$sql1.= "     FROM tbl_telefonia ";
	$sql1.= " GROUP BY tx_tipo ";
	$sql1.= " ORDER BY id_telefonia ";
	...
	switch ($reg_computo) 
	{   
		case 0 : $TheWidthCom	='20%'; $TheWidthDet='10%'; break;
		case 1 : $TheWidthCom	='18%'; $TheWidthDet='9%'; break;  
		case 2 : $TheWidthCom	='16%'; $TheWidthDet='8%'; break; 
		case 3 : $TheWidthCom	='12%'; $TheWidthDet='6%'; break; 
	...
	if ($registros==0) { 		
		...
		echo "<tr>";							
			...
			echo "<td colspan='$titulos' class='ui-state-highlight align-center'>TELEFONIA</td>";
		...
		for ($a=0; $a < count($TheCatalogoComputo); $a++) { 	        			 
			while ($elemento = each($TheCatalogoComputo[$a]))					  		
				$tx_tipo	=$TheCatalogoComputo[$a]['tx_tipo'];
				echo "<td width='$TheWidthCom' colspan='2' class='ui-state-highlight align-center'>$tx_tipo</td>";	
		}					
		echo "</tr>";					
		...
		for ($i=0; $i < count($TheCatalogoEmpleados); $i++)	{ 	        			 
			...
			for ($b=0; $b<4; $b++)
		 	{
				...
				{   
					...
					case 1:	if ($tx_indicador=="0") $TheColumn="<img src='images/redball.png' alt='$tx_notas'/>"; 
							...
							echo "<td class='align-center' valign='top' alt='ACTIVO'>$TheColumn</td>";
					break;
			...
			for ($a=0; $a < count($TheCatalogoComputo); $a++) { 	        			 
				while ($elemento = each($TheCatalogoComputo[$a]))	
					...
					$sql2 = "   SELECT count( * ) AS in_computo, SUM( fl_precio_usd ) + SUM( fl_precio_mxn / $tipo_cambio ) AS total_precio_usd ";
					$sql2.= "     FROM tbl_empleado a, tbl_empleado_telefonia b, tbl_telefonia c, tbl_telefonia_plan d ";
					$sql2.= "    WHERE a.id_empleado		= $id_empleado ";
					$sql2.= "      AND a.id_empleado 		= b.id_empleado ";
					$sql2.= "      AND b.id_telefonia 		= c.id_telefonia ";
					$sql2.= "      AND b.id_telefonia_plan 	= d.id_telefonia_plan ";
					$sql2.= "      AND tx_tipo 				= '$tx_tipo' ";
					$sql2.= " GROUP BY tx_tipo ";
			...
			for ($a=0; $a < count($TheCatalogoComputo); $a++) { 					        			 
				...
				if ($par_subdireccion==0) {
					...
					$sql = "   SELECT count( * ) AS in_computo_total, SUM( fl_precio_usd ) + SUM( fl_precio_mxn / $tipo_cambio ) AS total_precio_usd ";
					$sql.= " 	 FROM tbl_empleado a, tbl_centro_costos b, tbl_direccion c, tbl_subdireccion d, tbl_departamento e, tbl_empleado_telefonia f, tbl_telefonia g, tbl_telefonia_plan h ";
					$sql.= "    WHERE a.id_centro_costos  	= b.id_centro_costos ";
					$sql.= "	  AND b.id_direccion 		= $par_direccion ";
					$sql.= "	  AND b.id_direccion 		= c.id_direccion ";
					$sql.= "	  AND b.id_subdireccion 	= d.id_subdireccion ";
					...
					$sql.= "      AND f.id_telefonia 		= g.id_telefonia ";
					$sql.= "      AND f.id_telefonia_plan 	= h.id_telefonia_plan ";
					$sql.= "      AND g.tx_tipo 			= '$tx_tipo' ";
					if ($par_status=="2"){}
					else $sql.= "	  AND a.tx_indicador 	= $par_status ";
			...
			echo "<ul type='square'> ";
			echo "<li>Unidades calculada en base al Equipamiento de Telefon&iacute;a.</em></li> ";
			echo "<li>Monto calculado en base al Equipamiento de Telefon&iacute;a.</li> ";
			echo "<li>Monto en USD (D&oacute;lares Americano).</li>";
			echo "<li>Tipo de cambio 13.00 pesos por d&oacute;lar (Fuente: Control Econ&oacute;mico).</li>";
			echo "</ul>";      
			echo "</td>";
			echo "</tr>";
