...
<script type="text/javascript">	
	...
	function fieldsRequiredListaFactura(){
		...
		if (moneda=="USD" || moneda=="EUR") validText(false, $("#cap_tipo_cambio"), $("#cap_tipo_cambio"), 1);
		...
		if (moneda=="USD" || moneda=="EUR") {				
			if( !validText(false, $("#cap_factura"), $("#errcap_factura"), 1) || 
				...
				!validText(false, $("#cap_fh_inicial"), $("#errcap_fh_inicial"), 1) || 
				!validText(false, $("#cap_fh_final"), $("#errcap_fh_final"), 1) || 
			//	!validSelectCeros($("#cap_tipo_cambio"), $("#errcap_tipo_cambio")) ||
...
<?
	...
	if ($dispatch=="insert") {	
		...
		if ($ac_moneda==1) 		$fl_precio	= $fl_precio_mxn;
		else if ($ac_moneda==2) $fl_precio	= $fl_precio_usd;
		else if ($ac_moneda==3) $fl_precio	= $fl_precio_eur;
		...
		$sql.= "     FROM tbl_cuenta ";
		$sql.= " 	WHERE id_proveedor = $ac_proveedor  and tx_indicador='1'";		
	  	...
	  	<table cellspacing="1px" border="0" cellpadding="0" width="100%">   		
            ...
            <tr>
              	...
              	<td>
                    ...
                    <input name="id_moneda" id="id_moneda" type="hidden" size="10" value="<? echo $ac_moneda ?>"/>
              	</td>
            ...
            <tr>
            	<td class="ui-amarillo">Monto en MXN</td>
			  	...
			  	<?php
					if ($ac_moneda==2)	{				
              			echo "<input name='cap_monto_mxn' id='cap_monto_mxn' type='text' size='30' alt='signed-decimal-us' title='Monto en MXN' value='$fl_precio_mxn' />";
					} else {
                		echo "<input name='cap_monto_mxn' id='cap_monto_mxn' type='text' size='30' alt='signed-decimal-us' title='Monto en MXN' value='$fl_precio_mxn' disabled='disabled'/>";
       				}	 
				?>				</td>					
              	<td>&nbsp;</td>
            </tr>
            <tr>
              	...
              	<td><input name='cap_tipo_cambio' id='cap_tipo_cambio' type='text' size='30' alt='decimal' title='Tipo de Cambio' value="<? echo $fl_tipo_cambio ?>" disabled="disabled"/></td>
           	  	<td><div id="errcap_tipo_cambio" style="float:left;"></td>
            </tr>
            <tr>
              	<td class="ui-state-default">Fecha de la Factura:</td>
              	<td><input name="cap_fh_factura" id="cap_fh_factura" type="text" size="30" title="Fecha Factura" value="<? echo cambiaf_a_normal($fh_factura) ?>" pattern="dd/MM/yyyy"/></td>
           	  <td><div id="errcap_fh_factura" style="float:left;"></div></td>  
            </tr>
            <tr>
            	<td class="ui-state-default">Fecha Inicial:</td>
           	  	<td><input name="cap_fh_inicial" id="cap_fh_inicial" type="text" size="30" title="Fecha Inicial" value="<? echo cambiaf_a_normal($fh_inicio) ?>" pattern="dd/MM/yyyy"/></td>
       	  	  	<td><div id="errcap_fh_inicial" style="float:left;"></div></td>   
           	</tr>
            <tr>
            	<td class="ui-state-default">Fecha Final:</td>
           	  	<td>
                	<input name="cap_fh_final" id="cap_fh_final" type="text" size="30" title="Fecha Final" value="<? echo cambiaf_a_normal($fh_final) ?>" pattern="dd/MM/yyyy" />                </td>
       	  	  	<td width="20%"><div id="errcap_fh_final" style="float:left;"></div></td>   
           	</tr>
            <tr>
           	  	<td class="ui-amarillo">Fecha Contable:</td>
              	<td><input name="cap_fh_contable" id="cap_fh_contable" type="text" size="30" title="Fecha Contable" value="<? echo cambiaf_a_normal($fh_contable) ?>" pattern="dd/MM/yyyy" /></td>
           	  <td>&nbsp;</td>
            </tr>
            <tr>
